...
- **THEN** scoring item values and existing execution feedback rendering remain unchanged
...
- **THEN** the `info` block contains no measured FPS, SSIM, watermark recognition rate, color check rate, CPU mean percent, thresholds, run directory, Chromium version, full Playwright stack trace, raw forensic evidence arrays, or raw internal failure reason
- **THEN** those diagnostics, when available, are confined to `debug`, `score.json`, `report.html`, run logs, or profile artifacts
- **THEN** any `Execution Feedback:` lines and decode-path violation lines in `info` are concise, sanitized messages intended for contestants
