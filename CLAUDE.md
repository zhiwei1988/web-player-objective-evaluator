...
Organizer-internal automated scorer for the objective portion of a web plugin-free real-time media player challenge. Generates watermarked H.265 RTSP streams, drives each contestant submission through Playwright Chromium, compares decoded frames against watermarked references, and emits a contestant-visible `score.json` plus an internal `report.html`. Built on Ubuntu 24.04 with a source-built C/C++/Go toolchain (ffmpeg, mediamtx, tesseract, leptonica, libdmtx, x265) and Python 3.12.

Two execution modes:

- **Native build host** — clone, `setup.sh → build.sh → deploy.sh`, then `scripts/evaluator-local.sh`. Used for development and self-test.
- **Portable OCI bundle** — `scripts/package.sh` on the build host produces `dist/` (`docker save` archive + host wrapper + manifest). Any docker-equipped Ubuntu 24.04 host runs `evaluator-host.sh` against it; no apt install or source build at evaluation time.
...
| Portable bundle contract (manifest fields, target-host prereqs, sha-lock) | `openspec/specs/portable-bundle/spec.md` |
...
- **Contestant contract is frozen.** Changing the URL parameter, env vars, fixed RTSP source URLs, or DOM signals requires an OpenSpec change to `openspec/specs/evaluator/spec.md`. **The contestant runs on the host** (native process group), not inside the container, even when scored via `evaluator-host.sh`; the container only carries the evaluator's MediaMTX + Playwright + Python pipeline.
...
- **Reserved ports** are cleaned only by the wrappers and `scripts/teardown.sh`. Contestants are free to bind any other local port internally; we don't prescribe or clean them — the contestant's process group is SIGKILLed as a whole at cleanup. Playwright drives Chrome over its own internal CDP channel; no fixed remote-debugging port is exposed.

- **Idempotent scripts.** `setup.sh`/`build.sh`/`deploy.sh`/`package.sh`/`evaluator-host.sh`/`evaluator-local.sh`/`evaluator.sh` are all safe to re-run. The two evaluator wrappers are flock-mutex'd at `/var/tmp/evaluator-host.lock`; concurrent invocations exit 75 rather than corrupting each other's port state.
...
- **Path discipline.** Every shell script computes `SCRIPT_DIR = scripts/` then `ROOT_DIR = "${SCRIPT_DIR}/.."` and sources `scripts/env.sh`. **Exception**: `scripts/evaluator-host.sh` sets `ROOT_DIR="${PWD}"` so target-host operators can `cd` into any working directory and have `submissions/` / `results/` land there, decoupled from where the script binary lives.
