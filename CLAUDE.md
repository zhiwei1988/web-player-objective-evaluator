...
- **Contestant bandwidth limiter is per-run, owned by `scripts/evaluator.sh` + `scripts/_contestant_lifecycle.sh`.** The contestant process tree's IP-layer egress is physically shaped to `EVALUATOR_CONTESTANT_BANDWIDTH_MAX` (default `100mbit`) by tagging the contestant cgroup's packets with nftables `socket cgroupv2` and steering them into a tc/htb class on `lo` — reusing the same systemd transient unit that enforces the memory limit. Applied right after `clx_start_contestant` (the cgroup must exist first), torn down idempotently on every exit path via the cleanup trap (`clx_teardown_contestant_bandwidth_limit`). Needs `CAP_NET_ADMIN`, granted on system `tc`/`nft` by `scripts/build.sh::apply_net_admin_cap` (same setcap pattern as mediamtx). Real shaping is exercised by the capability-gated `tests/test_bandwidth_shaping_integration.py`; command shape by `tests/test_contestant_bandwidth_limit.py`.
